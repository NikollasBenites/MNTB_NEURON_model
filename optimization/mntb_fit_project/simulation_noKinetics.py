...
from neuron_model_noKinetics import set_conductances
...
def run_simulation(soma, axon, dend, params):
    ...
    set_conductances(soma, axon, dend, neuron_params, na_scale, kht_scale, klt_scale, ih_soma, ih_dend)
     ...
     # 1. Initialize
    h.v_init = config_noKinetics.v_init
    h.finitialize(h.v_init)
    # 2. Pre-run for stabilization
    h.dt = config_noKinetics.h_dt  # time step
    h.tstop = 1000  # ms (1 second or more)
   # run simulation with no stimulation
    ...
    stim.delay = config_noKinetics.stim_delay_ms
    stim.dur = config_noKinetics.stim_dur_ms
    stim.amp = stim_amp

    t_vec = h.Vector().record(h._ref_t)
    v_vec = h.Vector().record(soma(0.5)._ref_v)

    h.dt = config_noKinetics.h_dt
