...
module_path = '/Users/nikollas/Library/CloudStorage/OneDrive-UniversityofSouthFlorida/MNTB_neuron/mod/fit_final/optimization'
...
filename = "fit_gNa_vs_gKA_gNa_ratio_TeNT_average"
if os.path.exists(param_file_path):
    ...
    fixed_params = {
        ...
        'gka': params_row["gka"],# this will be updated later
        'ena': 62.77,
        'ek': -106.81,
        'cam': params_row["cam"],
        'kam': params_row["kam"],
...
gka_fixed = fixed_params['gka']
print(f"gKA fixed: {gka_fixed}")
ratio_fixed = gka_fixed / gna_fixed if gna_fixed != 0 else 0.0
# === Define ranges ===
...
#gna_values = np.unique(np.concatenate([gna_coarse, gna_fine]))#, gna_high
gna_values = np.linspace(40, 400, 50)        # Sodium conductance in nS
...
plt.xlabel('gNa (nS)')
plt.ylabel('gNa / gKA Ratio')
plt.title('Spike Count vs gNa and gNa/gKA Ratio')
plt.grid(False)
plt.tight_layout()
plt.show()
...
fixed_sim_params['gka'] = gka_fixed
