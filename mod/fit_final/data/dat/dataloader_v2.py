...
def is_voltage_clamp(trace_array):
    """Heuristic to detect voltage-clamp (currents in nA) vs current-clamp (voltages in mV)."""
    return np.max(np.abs(trace_array)) < 1.0  # Below ~1 mV = likely current (in nA)
...
voltage, time, stim, labels, n_sweeps, label_list, series = load_heka_data(
    full_path_to_file, group_idx=0, series_idx=1, channel_idx=0
)

# === Determine clamp mode
